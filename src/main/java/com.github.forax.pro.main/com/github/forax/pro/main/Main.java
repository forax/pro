...
public class Main {
  ...
  enum Command {
    ...
    static Command command(String name) {
      return Arrays.stream(values())
          ...
          .orElseThrow(
              () -> {
                throw new IllegalArgumentException("unknown sub command " + name);
              });
    }
  }
  ...
  static void build(String[] args) {
    ...
    ServiceLoader<ConfigRunner> loader =
        ServiceLoader.load(ConfigRunner.class, ConfigRunner.class.getClassLoader());
        ...
        .orElseThrow(
            () ->
                new IllegalStateException(
                    "no config runner available for config file " + configFile))
        .run();
  }

  static void daemon(String[] args) {
    Daemon service = getDaemon().orElseThrow(() -> new IllegalStateException("daemon not found"));
