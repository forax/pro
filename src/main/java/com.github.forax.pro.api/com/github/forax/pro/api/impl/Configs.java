...
public class Configs {
  ...
  private static <T> Optional<T> getProperty(
      Object proxy, String key, Class<T> type, boolean readOnly) {
  ...
  private static Object traverse(Object proxy, String[] properties, int count, String key) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      result =
          getProperty(result, property, Object.class, true)
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "property " + property + " in '" + key + "' is not defined"));
    }
    return result;
  }
  ...
  private static Object getFromMap(
    ...
    if (type == Optional.class) {
      ...
      value =
          map.computeIfAbsent(
              key,
              __ -> { // auto-vivification if possible
                if (readOnly || !type.isAnnotationPresent(TypeCheckedConfig.class)) {
                  throw new NoSuchElementException("no value for key " + key);
                }
                return proxy(
                    type, context, id.isEmpty() ? key : id + '.' + key, new HashMap<>(), false);
              });
    }
    ...
    try {
      ...
      SET_HASH =
          lookup.findStatic(
              Configs.class,
              "setFromMap",
              methodType(void.class, Map.class, Object.class, String.class, Class.class));
              ...
              case 1:
                {
                  MethodHandle mh =
                      MethodHandles.insertArguments(
                          SET_HASH, 2, name, method.getParameterTypes()[0]);
                  setterMap.put(name, mh);
                  continue;
                }
              default:
                throw new IllegalStateException(
                    "invalid method " + method + " on " + declaringClass.getName());
            }
          }
          @SuppressWarnings("unchecked")
              ...
              if (declaringClass == Query.class) {
                  ...
                  case "_set_":
                    {
                      if (proxyReadOnly) {
                        throw new UnsupportedOperationException(
                            "configuration is read only for type " + proxyClass.getSimpleName());
                      }
                      String key = (String) args[0];
                      Object value = args[1];
                  ...
                  case "_duplicate_":
                    {
                      ...
                      map.forEach(
                          (key, value) -> {
                            Object newValue =
                                (value instanceof Query) ? ((Query) value)._duplicate_(ctx) : value;
                ...
                if (declaringClass == Object.class) {
                  ...
                  if (parameterCount == 0) {
                    return getterMap
                        .get(name)
                        .invokeExact(context, id, map, method.getReturnType(), proxyReadOnly);
                  }
                  if (parameterCount == 1) {
                    if (proxyReadOnly) {
