...
public class FileHelper {
  ...
  public static <T, U, R> BiFunction<T, U, R> unchecked(
      IOBiFunction<? super T, ? super U, ? extends R> function) {
    return (element, element2) -> {
      try {
  ...
  public static List<Path> walkIfNecessary(List<Path> list, Predicate<? super Path> filter) {
    return list.stream()
        .flatMap(
            FileHelper.<Path, Stream<Path>>unchecked(
                path -> {
  ...
  public static void walkAndFindCounterpart(
      Path srcPath,
      Path dstPath,
      Function<Stream<Path>, Stream<Path>> configure,
