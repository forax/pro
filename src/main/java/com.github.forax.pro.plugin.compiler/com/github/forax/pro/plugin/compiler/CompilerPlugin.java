...
public class CompilerPlugin implements Plugin {
  ...
  public void configure(MutableConfig config) {
    ...
    derive(
        compiler,
        CompilerConf::moduleDependencyPath,
        convention,
        ConventionFacade::javaModuleDependencyPath);
    derive(
        compiler,
        CompilerConf::moduleSourcePath,
        convention,
        ConventionFacade::javaModuleSourcePath);
    derive(
        compiler,
        CompilerConf::moduleSourceResourcesPath,
        convention,
        ConventionFacade::javaModuleSourceResourcesPath);
    derive(
        compiler, CompilerConf::moduleTestPath, convention, ConventionFacade::javaModuleTestPath);
    derive(
        compiler,
        CompilerConf::moduleTestResourcesPath,
        convention,
        ConventionFacade::javaModuleTestResourcesPath);

    // outputs
    derive(
        compiler,
        CompilerConf::moduleExplodedSourcePath,
        convention,
        c -> c.javaModuleExplodedSourcePath().get(0));
    derive(
        compiler,
        CompilerConf::moduleMergedTestPath,
        convention,
        c -> c.javaModuleMergedTestPath().get(0));
    derive(
        compiler,
        CompilerConf::moduleExplodedTestPath,
        convention,
        c -> c.javaModuleExplodedTestPath().get(0));
  }
  ...
  static Optional<List<Path>> modulePathOrDependencyPath(
      Optional<List<Path>> modulePath,
      List<Path> moduleDependencyPath,
      List<Path> additionnalPath) {
    return modulePath
        .or(
            () ->
                Optional.of(
                    new StableList<Path>()
                        .appendAll(moduleDependencyPath)
                        .appendAll(additionnalPath)))
  ...
  enum JavacOption {
    ...
    UPGRADE_MODULE_PATH(
        actionMaybe("--upgrade-module-path", Javac::upgradeModulePath, File.pathSeparator)),
  ...
  public int execute(Config config) throws IOException {
    ...
    if (moduleTestFinder.findAll().isEmpty()) { // there is no test module-info defined
      log.info(
          compiler.moduleTestPath(),
          testPath ->
              "test: can not find any test modules in "
                  + testPath.stream().map(Path::toString).collect(Collectors.joining(", ")));
      return 0;
    }
    ...
    ModuleFinder moduleMergedTestFinder =
        ModuleHelper.sourceModuleFinder(compiler.moduleMergedTestPath());
    return compile(
        log,
        javacTool,
        compiler,
        ...
        StableList.<Path>of()
            .appendAll(compiler.moduleSourceResourcesPath())
            .appendAll(compiler.moduleTestResourcesPath()),
  ...
  private static int compile(
      ...
      List<Path> additionalSourcePath,
      List<Path> resourcesPath,
      Path destination,
      String pass)
      throws IOException {
    Optional<List<Path>> modulePath =
        modulePathOrDependencyPath(
            compiler.modulePath(), compiler.moduleDependencyPath(), additionalSourcePath);

    ModuleFinder dependencyFinder =
    ...
    if (rootSourceNames.isEmpty()) {
      log.error(
          ...
          sourcePath ->
              pass
                  + " can not find any modules in "
                  + sourcePath.stream().map(Path::toString).collect(Collectors.joining(", ")));
      return 1; //FIXME
    }

    ModuleFinder systemFinder =
        ModuleHelper.filter(
    ...
    */
    boolean resolved =
        ModuleHelper.resolveOnlyRequires(
            ModuleFinder.compose(moduleFinder, dependencyFinder, systemFinder),
            rootSourceNames,
            (moduleName, dependencyChain) -> {
              log.error(
                  null, __ -> pass + " can not resolve " + moduleName + " from " + dependencyChain);
            });
    if (!resolved) {
      return 1; //FIXME
    }
    ...
    CmdLine cmdLine =
        gatherAll(JavacOption.class, option -> option.action).apply(javac, new CmdLine());
    List<Path> files =
        compiler
            .files()
            .orElseGet(
                () ->
                    walkIfNecessary(
                        moduleSourcePath, pathFilenameEndsWith(".java"))); //FIXME, use rootNames ??
    files.forEach(cmdLine::add);
    String[] arguments = cmdLine.toArguments();
    log.verbose(
        files,
        fs ->
            OptionAction.toPrettyString(JavacOption.class, option -> option.action)
                    .apply(javac, "javac")
                + "\n"
  ...
  private static int merge(
      ModuleFinder moduleSourceFinder, ModuleFinder moduleTestFinder, Path moduleMergedTestPath)
      throws IOException {
    Files.createDirectories(moduleMergedTestPath);

    Predicate<Path> skipModuleInfoDotJava =
        FileHelper.pathFilenameEquals("module-info.java").negate();
    ...
    for (ModuleReference testRef : moduleTestFinder.findAll()) {
      ...
      if (sourceRefOpt.isPresent()) {
        ...
        FileHelper.walkAndFindCounterpart(
            sourcePath, moduleRoot, stream -> stream.filter(skipModuleInfoDotJava), Files::copy);

        ModuleDescriptor descriptor =
            ModuleHelper.mergeModuleDescriptor(sourceRef.descriptor(), testRef.descriptor());
        Files.write(
            moduleRoot.resolve("module-info.java"),
            List.of(ModuleHelper.moduleDescriptorToSource(descriptor)));
